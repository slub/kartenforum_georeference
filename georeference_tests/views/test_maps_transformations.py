...
def test_GET_TransformationsForMapId_success_emptyResult(testapp):
    ...
    # Build test request
    res = testapp.get(ROUTE_PREFIX + '/maps/%s/transformations' % map_id, status=200)
    assert res.status_int == 200
    assert len(res.json['items']) == 0

def test_GET_TransformationsForMapId_success_transformationResults(testapp, dbsession):
    map_id = 10001556
...
def test_POST_TransformationForMapId_success_newTransformation(testapp, dbsession):
    ...
    # Build test request
    res = testapp.post(ROUTE_PREFIX + '/maps/%s/transformations' % map_id, params=json.dumps(params), content_type='application/json; charset=utf-8', status=200)
