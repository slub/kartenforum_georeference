...
def get_crs_for_transformation_params(transformation_params, raw_map_obj, target_crs=None):
...
def transform_to_params_to_target_crs(transformation_params, target_crs):
    ...
    if target_crs.upper() != str(transformation_params['target']).upper():
        transformer = pyproj.Transformer.from_crs(transformation_params['target'], target_crs, always_xy=True)
        transformed_gcps = map(
            lambda p: {'source': p['source'], 'target': transformer.transform(p['target'][0], p['target'][1])},
