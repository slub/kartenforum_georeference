...
MAPPING = {
    ...
    "keywords": {
        "type": "text",
        "index": False,
    },  # "Druckgraphik;Lithografie & Umdruck"
    "title_long": {
        "type": "text",
        "index": False,
    },  # "Me\u00dftischblatt 119 : Altenberg, 1939"
    "title": {"type": "keyword", "index": True},  # Altenberg
    "permalink": {
        "type": "text",
        "index": False,
    },  # "http://digital.slub-dresden.de/id335921620"
    "slub_url": {
        "type": "text",
        "index": False,
    },  # "http://digital.slub-dresden.de/id335921620"
    ...
    "is_mosaic": {
        "type": "boolean",
        "index": True,
    },  # Signals if the given map is a mosaic map layer
}
...
def _get_online_resource_wcs_for_download(georef_map_obj, coverage_title, extent):
    ...
    return {
        ...
        % (
            {
                "link_service": TEMPLATE_PUBLIC_WCS_URL.format(
...
def generate_es_original_map_document(
    raw_map_obj, metadata_obj, georef_map_obj=None, geometry=None
):
    ...
    try:
        ...
        if georef_map_obj is not None and os.path.exists(georef_map_obj.get_abs_path()):
            ...
            if raw_map_obj.allow_download:
                ...
                online_resources.append(
                    _get_online_resource_wcs_for_download(
                        georef_map_obj,
        ...
        if georef_map_obj is not None and os.path.exists(georef_map_obj.get_abs_path()):
            for template in TEMPLATE_TMS_URLS:
                tms_urls.append(
                    template.format(
                        str(raw_map_obj.map_type).lower() + "/" + raw_map_obj.file_name
                    )
                )
        ...
        keywords = ";".join(
            list(
                filter(
                    lambda x: x is not None, [metadata_obj.type, metadata_obj.technic]
                )
            )
        )
        ...
        return {
            ...
            "map_scale": int(raw_map_obj.map_scale)
            if raw_map_obj.map_scale is not None
            else None,
...
def generate_es_mosaic_map_document(mosaic_map_obj, geometry=None):
    ...
    try:
        ...
        return {
            ...
            "map_scale": int(mosaic_map_obj.map_scale)
            if mosaic_map_obj.map_scale is not None
            else None,
            "zoomify_url": None,
            "map_type": None,
            "keywords": "",
...
def get_es_index(es_config, index_name, force_recreation):
    ...
    try:
        ...
        if index_exists is False or force_recreation:
            logger.debug(
                'Index "%s" does not exist. Create a fresh index.' % index_name
            )
            es.indices.create(
                index=index_name,
...
if __name__ == "__main__":
    get_es_index({"host": "localhost", "port": 9200}, "vk20", True)
