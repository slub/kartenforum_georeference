...
def run_process_new_validation(es_index, dbsession, job):
    ...
    transformation_obj.validation = (
        EnumValidationValue.VALID.value
        if job.type == EnumJobType.TRANSFORMATION_SET_VALID.value
        else EnumValidationValue.INVALID.value
    )
    transformation_obj.comment = (
        jobs_desc["comment"] if "comment" in jobs_desc else None
    )

    logger.debug(
        "Set transformation %s to %s."
        % (transformation_obj.id, transformation_obj.validation)
    )
    ...
    if (
        job.type == EnumJobType.TRANSFORMATION_SET_VALID.value
        and GeorefMap.by_raw_map_id(transformation_obj.raw_map_id, dbsession) is None
    ):
        ...
        run_enable_transformation(transformation_obj, es_index, dbsession)
        ...
        prev_transformation_obj = Transformation.query_previous_valid_transformation(
            transformation_obj, dbsession
        )

        # If there is no previous valid transformation, but a currently active georef map, then disable it.
        active_georef_map_obj = GeorefMap.by_transformation_id(
            transformation_obj.id, dbsession
        )
        if prev_transformation_obj is None and active_georef_map_obj is not None:
            logger.debug(
                "Disable georef map for transformation with id %s ..."
                % transformation_obj.id
            )
            run_disable_transformation(transformation_obj, es_index, dbsession)
        ...
        if prev_transformation_obj is not None and active_georef_map_obj is not None:
            logger.debug(
                "Enable older transformation with id %s ..."
                % prev_transformation_obj.id
            )
            run_enable_transformation(prev_transformation_obj, es_index, dbsession)
