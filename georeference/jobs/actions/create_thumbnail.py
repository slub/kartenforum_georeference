...
def run_process_thumbnail(path_src_raw_img, path_thumbnail, logger, width=None, height=None, force=False):
    ...
    if os.path.exists(path_thumbnail) and force == False:
        logger.debug('Skip processing of thumbnail "%s", because file already exists. Use "force" parameter in case you want to overwrite it.' % path_thumbnail)
    ...
    if width == None and height == None:
        raise Exception('Can not create a thumbnail, because width and height are both missing.')

    logger.info("Process thumbnail %s ..." % path_src_raw_img)

    command = "vipsthumbnail {path_src} -o {path_trg}[strip] --size {width}x{height}".format(
        path_src=path_src_raw_img,
        path_trg=path_thumbnail,
        width=width if width != None else "",
